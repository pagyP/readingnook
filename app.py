...
def generate_recovery_code_display_token(user_id, recovery_codes):
    """Generate a one-time server-side token that references recovery codes.
    
    Instead of storing all recovery codes in the session (increasing attack surface),
    we store them in a server-side cache and only reference them via a token.
    This token expires after 5 minutes - enough time for the user to view and save codes.
    
    Security benefit:
    - If session is compromised, attacker only gets a token, not the actual codes
    - Codes are cleared from cache after viewing or after 5 minutes
    - Reduces exposure window compared to storing all codes in session
    
    Args:
        user_id: ID of the user whose codes are being displayed
        recovery_codes: List of plain-text recovery codes to store
    
    Returns:
        One-time token that references the codes in the server cache
    """
...
def get_recovery_codes_from_cache(token):
    """Retrieve recovery codes from cache and remove the entry (one-time use).
    
    Args:
        token: One-time token generated by generate_recovery_code_display_token
    ...
    # Remove from cache after reading (one-time use)
    del recovery_code_cache[token]
    
    return cache_entry['user_id'], cache_entry['codes']

def cleanup_expired_recovery_codes():
    """Clean up expired entries from recovery code cache.
    
    This should be called periodically to prevent memory leaks.
    Called during registration and recovery code display.
    """
    now = datetime.now(timezone.utc)
    expired_tokens = [
        token for token, entry in recovery_code_cache.items()
...
def register():
    ...
    if form.validate_on_submit():
        ...
        # Store codes in server-side cache (safer than session)
        # Only reference via token - codes are not in session
        display_token = generate_recovery_code_display_token(user.id, recovery_codes)
        session['recovery_code_token'] = display_token  # Only token in session, not codes
...
def show_recovery_codes():
    ...
    Security: Recovery codes are stored in a server-side cache (not session) with:
    - 5-minute expiration
    - One-time access (removed from cache after viewing)
    - Only a reference token stored in session
    ...
    # Retrieve codes from cache and remove entry (one-time use)
