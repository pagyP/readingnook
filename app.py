...
def derive_encryption_key(password, salt=b'totp-secret'):
    """Derive a 32-byte encryption key from password using PBKDF2.
    
    Args:
        password: User's plaintext password
        salt: Salt for key derivation (default: 'totp-secret')
...
def encrypt_totp_secret(secret, password):
    ...
    Security:
        ...
        - Same password always derives same key, enabling decryption later
    """
...
def decrypt_totp_secret(encrypted_secret, password):
...
def setup_mfa():
    ...
    if form.validate_on_submit():
        ...
        if verification_result:
            # Encrypt secret with user's password
            encrypted_secret = encrypt_totp_secret(secret, form.password.data)
